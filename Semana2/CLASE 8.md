...
export const transferPostOwnership = async (postId: string, newAuthorId: string) => {
  ...
  try {
    ...
    Nota de Arquitectura: Las transacciones son el ejemplo perfecto de por qué usamos Servicios. El controlador no debería saber nada sobre "commits" o "rollbacks", solo llama a transferPostOwnership() y espera un éxito o un error.
