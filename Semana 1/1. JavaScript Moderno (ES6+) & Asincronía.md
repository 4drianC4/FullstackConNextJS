...
const obtenerUsuarios = async () => {
    try {
        ...
        const respuesta = await fetch("[https://jsonpla>[!INFO]  NO ES LO MISMO MANEJAR FUNCIONES SINCRONAS QUE ASINCRONAS
>Las funciones **síncronas** ejecutan tareas secuencialmente, bloqueando la ejecución hasta finalizar cada una, ideal para tareas dependientes. 
>Las funciones **asíncronas** permiten iniciar tareas (como consultas a red o archivos) y continuar sin esperar, mejorando la eficiencia y evitando bloqueos. Ambas mejoran la experiencia del usuario, siendo `async/await` comunes en asincronía. ceholder.typicode.com/users/1](https://jsonplaceholder.typicode.com/users/1)");
